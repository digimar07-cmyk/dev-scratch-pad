...
class OllamaClient:
    ...
    def __init__(self, active_models=None):
        ...
        self.active_models = active_models or dict(OLLAMA_MODELS)
    ...
    def _get_model(self, role):
        """
        Retorna nome do modelo configurado para o papel dado.
        """
        return self.active_models.get(role, OLLAMA_MODELS.get(role, ""))
    
    def _get_timeout(self, role):
        """
        Retorna timeout (connect, read) para o papel dado.
        """
    ...
    def generate_text(
        ...
        Usa /api/chat para melhor instruction-following (Qwen2.5-Instruct).
        
        Args:
            prompt: Texto do prompt
            role: Papel do modelo (text_quality, text_fast)
            temperature: Temperatura de sampling (0.0-1.0)
            num_predict: Máximo de tokens a gerar
        
        Returns:
            String com resposta do modelo ou "" em caso de erro.
        """
        if self.stop_flag:
            return ""
        ...
        # Payload para /api/chat (melhor para instruct models)
        payload = {
            "model": model,
            "messages": [
                {
    ...
    def describe_image(self, image_path):
        """
        Analisa imagem usando moondream.
        Usa /api/generate com imagem em base64.
        
        Args:
            image_path: Caminho para arquivo de imagem
        
        Returns:
            String com descrição da imagem ou "" em caso de erro.
        """
        if self.stop_flag:
            return ""
        ...
        try:
            ...
            # Prompt cirúrgico: foco no objeto central
            payload = {
                "model": model,
                "prompt": (
