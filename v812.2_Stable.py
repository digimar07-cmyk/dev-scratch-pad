...
Base: v8.1.2.1
Patches:
  1 - Icones de acao (fav/done/good/bad) atualizam cor no card sem recriar a lista
  2 - Proporcao do card corrigida para 3:2 (4496x3000 = 1.498 ~= 3/2)
  3 - Menu de analise reestruturado em 4 acoes claras + Reanalisar com nova descricao forcada
  4 - Modal detalhe: chips/titulo sobem, descricao ocupa mais espaco, fonte 15px
"""
...
Tags: tag1, tag2, tag3, tag4, tag5, tag6, tag7, tag8"""
    ...
    def generate_description(self, project_path, data, force_new=False):
        ...
        try:
            ...
            if getattr(self, "stop_analysis", False): return None
            # force_new: temperatura maior e instrucao explicita de ser diferente da versao anterior
    ...
    def run_analysis_thread(self, projects_list, on_progress=None, on_done=None, with_description=False, force_desc=False):
        ...
        def _run():
            ...
            for i, path in enumerate(projects_list, 1):
                ...
                self.save_database()
                # Se solicitado, gera descricao em sequencia
...
def api_toggle():
    ...
    # Retorna estado completo do produto para o frontend atualizar o card
    d = engine.database.get(pp, {})
...
def api_analyze():
    ...
    with_desc    = data.get("with_description", False)   # Patch 3: analise completa
    force_desc   = data.get("force_description", False)  # Patch 3: forcra nova desc no reanalisar
...
def api_describe():
    ...
    if mode == "single":
        ...
        def _gen():
            ...
            engine.generate_description(pp, d, force_new=True)  # sempre force_new no single
...
/* ── DROPDOWN — v8.1.2.2 menu reestruturado ── */
...
/* PATCH 2: proporcao 3:2 (4496x3000 = 1.4987 ~ 3/2) */
...
/* PATCH 1: botoes de acao com estados visuais corretos */
...
/* ══════════════════════════════════════════════════════════
   MODAL FULL-SCREEN — Netflix Style  (v8.1.2.2 — PATCH 4)
   ══════════════════════════════════════════════════════════ */
#modal-bg{
...
/* PATCH 4 — Conteudo ocupa altura toda, flexbox column justify-end */
#modal-content-fs{
  position:absolute;
  inset:0;right:auto;
  width:58%;max-width:760px;
  padding:72px 56px 36px 56px;
  z-index:10;
...
.fs-meta{
  ...
  animation:mSlideUp .35s ease .05s both;
}
.fs-cat-chip{
...
.fs-title{
  ...
  margin-bottom:6px;
  animation:mSlideUp .35s ease .1s both;
}
.fs-origin{
  font-size:13px;color:rgba(255,255,255,.5);
  margin-bottom:14px;letter-spacing:.5px;
  animation:mSlideUp .35s ease .12s both;
}

.fs-actions{
  display:flex;gap:8px;flex-wrap:wrap;margin-bottom:16px;
  animation:mSlideUp .35s ease .15s both;
}
.fs-btn{
  ...
  cursor:pointer;transition:transform .15s,background .15s;border:none;
}
...
/* PATCH 4 — Descricao: max-height maior e fonte 15px */
.fs-desc-wrap{
  flex:1;max-height:200px;overflow-y:auto;
  margin-bottom:14px;
  animation:mSlideUp .35s ease .18s both;
}
...
.fs-gen-btn{
  ...
  animation:mSlideUp .35s ease .2s both;
...
.thumb-item{
  width:72px;height:54px;object-fit:cover;border-radius:4px;cursor:pointer;
  border:2px solid transparent;transition:border-color .15s,transform .15s,opacity .15s;
  opacity:.55;flex-shrink:0;
}
.thumb-item:hover{opacity:.85;transform:scale(1.05)}
.thumb-item.active{border-color:#fff;opacity:1}
.thumb-ph{width:72px;height:54px;border-radius:4px;background:var(--surface3);
  display:flex;align-items:center;justify-content:center;font-size:22px;
  border:2px solid transparent;flex-shrink:0;opacity:.4}
...
<!-- DROPDOWN — PATCH 3: menu reestruturado em 4 acoes claras -->
...
<div id="modal-bg">
  ...
  <div id="modal-content-fs">
    ...
    <button class="fs-gen-btn" id="fs-gen-btn" onclick="generateDesc()">
      &#9997;&#65039; Gerar Descri&#231;&#227;o
    </button>
  </div>
</div>
...
let searchTimer;
function onSearch(v){ clearTimeout(searchTimer); searchTimer=setTimeout(()=>{state.search=v;loadProducts();},300); }
...
async function loadProducts(){
  const params = new URLSearchParams({filter:state.filter,origin:state.origin,categories:state.categories.join(','),tag:state.tag||'',search:state.search});
...
function renderCards(items){
  ...
  if(!items.length){
    grid.innerHTML=`<div class="empty-state" style="grid-column:1/-1"><div class="es-icon">&#128230;</div><h2>Nenhum produto encontrado</h2><p>Tente outros filtros ou adicione uma pasta.</p><button class="es-btn" onclick="addFolder()">&#128194; Adicionar Pasta</button></div>`;
    return;
  }
  grid.innerHTML='';
  items.forEach(p => {
    ...
    const imgHtml = p.has_cover
      ? `<img class="card-img" src="/api/cover?path=${encodeURIComponent(p.id)}&idx=0" alt="${escHtml(p.name)}" loading="lazy" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'"><div class="card-img-ph" style="display:none">&#128230;</div>`
    ...
    /* PATCH 1 — classe 'on' controla visibilidade; sem recriar o card no toggle */
    div.innerHTML=`
      ${imgHtml}
      <div class="card-info">
...
// PATCH 1 — toggle cirurgico: apenas muda a classe 'on' do botao clicado
async function toggleFlag(e, pid, key, btn){
  ...
  // Atualiza apenas os botoes do card — sem recriar nada
...
function _renderModalFS(p){
  ...
  document.getElementById('fs-origin').innerHTML  = p.origin ? '&#127757;&#65039; '+escHtml(p.origin) : '';
...
async function modalToggle(key){
  ...
  _renderFSActions(p);
  // Sincroniza o card na lista principal se estiver visivel
...
async function generateDesc(){
  ...
  btn.disabled=true; btn.innerHTML='&#9203;&#65039; Gerando...';
...
// Apenas categorias e tags — somente novos
...
// Apenas descricoes — somente novos (sem force — nao sobrescreve existentes)
...
// Analisar novos completo: cats + tags + desc de uma vez
async function doAnalyzeFull(){
  ...
  if(!r||r.error){ showToast('&#10060; '+(r&&r.error?r.error:'Erro')); return; }
  if(r.count===0){ showToast('&#10004; Nenhum produto novo para analisar'); return; }
  showToast('&#129302; Analise completa de '+r.count+' produto(s)...');
  startProgress();
}
// Reanalisar tudo: forcca nova descricao diferente da anterior
