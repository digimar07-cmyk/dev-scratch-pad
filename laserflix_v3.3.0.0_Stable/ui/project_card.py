...
Função pura: recebe parent + dados + callbacks, retorna nada.
Não é classe, não guarda estado. Teto: 120 linhas.
"""
...
def build_card(
    ...
    """
    Renderiza um card no grid do parent.

    Callbacks esperados em `cb`:
        on_open_modal(path)
        on_toggle_favorite(path, btn)
        on_toggle_done(path, btn)
        on_toggle_good(path, btn)
        on_toggle_bad(path, btn)
        on_analyze_single(path)
        on_open_folder(path)
        on_set_category(cats_list)
        on_set_tag(tag)
        on_set_origin(origin)
    """
    ...
    # --- Capa ---
    cover_frm = tk.Frame(card, bg=BG_SECONDARY, width=CARD_W, height=COVER_H)
    cover_frm.pack(fill="x")
    cover_frm.pack_propagate(False)
    cover_frm.bind("<Button-1>", lambda e: cb["on_open_modal"](project_path))
    ...
    if cover_img:
        ...
        lbl.bind("<Button-1>", lambda e: cb["on_open_modal"](project_path))
    else:
        ph = tk.Label(cover_frm, text="📁", font=("Arial", 52),
                      bg=BG_SECONDARY, fg=FG_TERTIARY, cursor="hand2")
        ph.pack(expand=True)
        ph.bind("<Button-1>", lambda e: cb["on_open_modal"](project_path))
    ...
    nl.pack(anchor="w")
    nl.bind("<Button-1>", lambda e: cb["on_open_modal"](project_path))
