...
class LaserflixMainWindow:
    ...
    def _adapt_header_to_screen(self):
        """
        Ajusta tamanho de fonte e visibilidade do nav conforme resolução.
        Chamado UMA VEZ após create_ui().
        Garante que o header nunca some em telas menores (ex: notebook).
        """
        sw = self.root.winfo_screenwidth()

        # Em telas <= 1366px (notebook HD comum) reduz fontes dos botões extras
        if sw <= 1366:
            small_font = ("Arial", 9, "bold")
            normal_font = ("Arial", 9)
            for btn in getattr(self, "_header_btns", []):
                try:
                    btn.config(font=small_font, padx=8, pady=6)
                except Exception:
                    pass
            for btn in getattr(self, "_nav_btns", []):
                try:
                    btn.config(font=normal_font, padx=6)
                except Exception:
                    pass

        # Em telas <= 1200px esconde nav de filtro rápido
        # (continua acessível via sidebar)
        if sw <= 1200:
    ...
    def create_ui(self):
        ...
        # --- Nav filtros rápidos (referência para responsividade) ---
        ...
        for text, ftype in [
            ...
            btn = tk.Button(
                ...
                bg="#000000", fg=FG_PRIMARY, font=("Arial", 11),
        ...
        tk.Entry(
            ...
            bg="#333333", fg=FG_PRIMARY, font=("Arial", 11),
            width=26, relief="flat", insertbackground=FG_PRIMARY,
        ).pack(side="left", padx=4, ipady=5)

        # --- Botões extras (direita, antes da busca) ---
        extras_frame = tk.Frame(header, bg="#000000")
        extras_frame.pack(side="right", padx=6)
        self._header_btns = []

        # ── ⚙️ Menu (banco/config) ──────────────────────────────────────
        menu_btn = tk.Menubutton(
            extras_frame, text="⚙️ Menu",
            bg="#444444", fg=FG_PRIMARY, font=("Arial", 11, "bold"),
            relief="flat", cursor="hand2", padx=12, pady=8,
        )
        menu_btn.pack(side="left", padx=4)
        self._header_btns.append(menu_btn)
        ...
        # Fallback para telas muito pequenas: filtros rápidos
        menu.add_separator()
        menu.add_command(label="🏠 Home",       command=lambda: self.set_filter("all"))
        menu.add_command(label="⭐ Favoritos",  command=lambda: self.set_filter("favorite"))
        menu.add_command(label="✓ Feitos",      command=lambda: self.set_filter("done"))
        menu.add_command(label="👍 Bons",       command=lambda: self.set_filter("good"))
        menu.add_command(label="👎 Ruins",      command=lambda: self.set_filter("bad"))

        # ── 📂 Preparar Pastas ─────────────────────────────────────────
        prep_btn = add_prepare_button(self, extras_frame)
        prep_btn.pack(side="left", padx=4)
        self._header_btns.append(prep_btn)

        # ── 📥 Importação (dropdown: individual + em massa) ────────────
        import_btn = tk.Menubutton(
            extras_frame, text="📥 Importação",
            bg=ACCENT_RED, fg=FG_PRIMARY, font=("Arial", 11, "bold"),
            relief="flat", cursor="hand2", padx=12, pady=8,
        )
        import_btn.pack(side="left", padx=4)
        self._header_btns.append(import_btn)
        import_menu = tk.Menu(
            import_btn, tearoff=0, bg=BG_CARD, fg=FG_PRIMARY,
            activebackground=ACCENT_RED, activeforeground=FG_PRIMARY,
        )
        import_btn["menu"] = import_menu
        import_menu.add_command(
            label="➕ Importação Individual",
        ...
        # ── 🤖 Análise IA (dropdown: novos + todos) ────────────────────
        ai_btn = tk.Menubutton(
            extras_frame, text="🤖 Análise",
            bg=ACCENT_GREEN, fg=FG_PRIMARY, font=("Arial", 11, "bold"),
            relief="flat", cursor="hand2", padx=12, pady=8,
        )
        ai_btn.pack(side="left", padx=4)
        self._header_btns.append(ai_btn)
        ai_menu = tk.Menu(
            ai_btn, tearoff=0, bg=BG_CARD, fg=FG_PRIMARY,
            activebackground=ACCENT_RED, activeforeground=FG_PRIMARY,
        )
        ai_btn["menu"] = ai_menu
        ...
        # ── 📝 Descrições (dropdown: novos + todos) ────────────────────
        desc_btn = tk.Menubutton(
            extras_frame, text="📝 Descrições",
            bg="#3A7BD5", fg=FG_PRIMARY, font=("Arial", 11, "bold"),
            relief="flat", cursor="hand2", padx=12, pady=8,
        )
        desc_btn.pack(side="left", padx=4)
        self._header_btns.append(desc_btn)
        desc_menu = tk.Menu(
            desc_btn, tearoff=0, bg=BG_CARD, fg=FG_PRIMARY,
            activebackground=ACCENT_RED, activeforeground=FG_PRIMARY,
        )
        ...
        self.status_bar = tk.Label(
            self.status_frame, text="Pronto.",
            bg="#000000", fg=FG_SECONDARY, font=("Arial", 10), anchor="w",
        )
        ...
        self.progress_bar = ttk.Progressbar(
            self.status_frame, mode="determinate",
            length=300, style="G.Horizontal.TProgressbar",
        )
        self.stop_btn = tk.Button(
            self.status_frame, text="⏹ Parar",
    ...
    def _on_import_complete(self, success: int, failed: int):
        ...
        BUG-FIX: db_manager.save_database() chamado ANTES de atualizar a UI
        para garantir persistência mesmo que o app seja fechado logo depois.
        """
        # ── SALVA EM DISCO (era o bug: dados sumiam ao fechar o app) ──
        self.db_manager.save_database()

        msg = f"✅ Importação concluída: {success} produto(s) adicionado(s)"
        if failed > 0:
    ...
    def create_sidebar(self, parent):
        ...
        for title, attr in [
            ...
            tk.Label(
                self.sidebar_content, text=title,
                font=("Arial", 14, "bold"), bg=BG_SECONDARY, fg=FG_PRIMARY, anchor="w",
    ...
    def update_categories_list(self):
        ...
        if not all_cats:
            tk.Label(
                self.categories_frame, text="Nenhuma categoria",
    ...
    def update_tags_list(self):
        ...
        if not tags_sorted:
            tk.Label(
                self.tags_frame, text="Nenhuma tag",
                bg=BG_SECONDARY, fg=FG_TERTIARY, font=("Arial", 10, "italic"),
                anchor="w", padx=15, pady=10,
            ).pack(fill="x")
            return
    ...
    def display_projects(self):
        ...
        tk.Label(
            self.scrollable_frame, text=title_text,
        ...
        tk.Label(
            self.scrollable_frame, text=f"{len(filtered)} projeto(s)",
        ...
        if not filtered:
            tk.Label(
                ...
                text="Nenhum projeto.\nClique em 'Importação' para adicionar.",
    ...
    def create_project_card(self, project_path, data, row, col):
        card = tk.Frame(self.scrollable_frame, bg=BG_CARD, width=CARD_W, height=CARD_H)
        ...
        if cover_image:
            lbl = tk.Label(cover_frame, image=cover_image, bg=BG_SECONDARY, cursor="hand2")
        ...
        nl = tk.Label(
            info, text=nm, font=("Arial", 10, "bold"),
            bg=BG_CARD, fg=FG_PRIMARY, wraplength=CARD_W - 20,
        ...
        if cats:
            ...
            for i, cat in enumerate(cats[:CARD_MAX_CATEGORIES]):
                c = CATEGORY_COLORS[i]
                b = tk.Button(
                    cf, text=cat[:CARD_CATEGORY_MAX_LENGTH],
    ...
    # =========================================================================
    # ADD FOLDERS (importação individual)
    # =========================================================================

    def add_folders(self):
        while True:
            folder = filedialog.askdirectory(
                title="Selecione pasta com projetos laser", mustexist=True)
            if not folder:
                break
    ...
    def open_project_modal(self, project_path):
        ...
        def _gen_desc():
            ...
            def _generate_in_thread():
                try:
                    ...
                    modal.after(0, lambda: desc_lbl.config(text="❌ Erro ao gerar descrição", fg="#EF5350"))
        ...
        gen_btn = tk.Button(
            lp, text="🤖  Gerar com IA", command=_gen_desc,
        ...
        for lbl_t, lbl_c, present in [
            ...
            tk.Label(
                fmt_row, text=lbl_t, font=("Arial", 9, "bold"),
        ...
        cp_btn = tk.Button(
            lr, text="📋 Copiar", command=_copy_path,
    ...
    def open_edit_mode(self, parent_modal, project_path, data):
        ...
        tags_listbox = tk.Listbox(
            tags_list_frame, bg=BG_CARD, fg=FG_PRIMARY, font=("Arial", 10),
        ...
        for text, cmd, color in [
            ...
            tk.Button(
                tags_buttons_frame, text=text, command=cmd,
        ...
        tk.Button(
            final_buttons, text="💾 Salvar e Fechar",
        ...
        tk.Button(
            final_buttons, text="✕ Cancelar", command=edit_win.destroy,
