"""Vision - análise de imagens com moondream"""
import os
import base64
import io
...
class VisionAnalyzer:
    ...
    def _ollama_describe_image(self, image_path: str) -> str:
        if not image_path or not os.path.exists(image_path):
            return ""
        if not self.ollama._ollama_is_available():
            return ""

        model   = self.ollama._model_name("vision")
        timeout = self.ollama._timeout("vision")
        try:
            with Image.open(image_path) as img:
            ...
            resp = self.ollama.http_session.post(
                f"{self.ollama.ollama_base_url}/api/generate",
                json=payload,
                timeout=timeout,
            )
